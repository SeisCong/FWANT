...
print *,'  file list of GRAM-MATRIX='
read(*,'(a)') fnm_list
write(*,*) " ",trim(fnm_list)
...
call init_data_damp(fnm_list,num_cmp,nsta,nevt,  &
     num_dat,num_sta,num_evt,num_mval,num_xval,  &
...
subroutine alloc_var(nst,nev,nx,nbd)
  integer :: nst,nev,nx,nbd
  if (nst>0) then
...
subroutine init_data_damp(filenm,ncmp,nst,nev, &
    max_dat,max_st,max_ev,max_mval,max_xval,   &
    max_siz,max_row,max_bd,max_nel)
character (len=*),intent(in) :: filenm
integer,intent(in) :: ncmp,nst,nev
integer,intent(out) ::                     &
  max_dat,max_st,max_ev,max_mval,max_xval, &
  max_siz,max_row,max_bd,max_nel
character (len=132) :: fnm_k
integer :: idst(nst),idev(nev)
...
max_mval=nblk*ncmp
max_xval=max_mval+max_st+max_ev
max_nel=max_nel+(nst+nev)*max_dat +max_xval
max_row=max_dat+max_xval
max_bd=max_bd+nst+nev
...
real(DP) :: vmin_sta,vmax_sta,vmin_evt,vmax_evt
...
do i=num_mval+1,num_mval+num_sta
   write(79,'(i5,f10.5,f10.5)') i-num_mval,mval(i),mval(i)*weig_sta
end do
close(79)
...
do i=num_mval+num_sta+1,num_mval+num_sta+num_evt
   write(80,'(i5,f10.5,f10.5)') i-num_mval-num_sta,mval(i),mval(i)*weig_evt
end do
close(80)

...
integer(kind=INT_KIND) :: imultis
integer(kind=INT_KIND) :: num_cmp,nsta,nevt,nloc
integer(kind=INT_KIND) :: nrow_smoth,nbd_smoth,nel_smoth
! integer ::                                     &
...
integer(kind=INT_KIND)::num_nel
integer(kind=INT_KIND),allocatable :: idsta(:),idevt(:),idloc(:)
real(DP),allocatable :: wgloc(:)

integer(kind=INT_KIND) :: nit
integer::i,j,k,m,n,ncoef
integer(kind=INT_KIND)::nael
integer(kind=INT_KIND) :: listid,gid,ierr
...
subroutine alloc_var(nst,nev,nlo,nx,nbd)
  integer(kind=INT_KIND) :: nst,nev,nlo
  integer :: nx,nbd
  if (nst>0) then
...
integer(kind=INT_KIND),intent(in) :: ncmp
integer(kind=INT_KIND),intent(out) :: max_row,max_bd,max_nel
integer(kind=INT_KIND) :: fid,nblk,ierr
...
character (len=*),intent(in) :: filenm
integer(kind=INT_KIND),intent(in) :: ncmp,nst,nev,nlo
integer,intent(out) ::                     &
  max_dat,max_st,max_ev,max_lo,max_mval,max_xval, &
  max_siz,max_row,max_bd !,max_nel
integer(kind=INT_KIND),intent(out)::max_nel
character (len=132) :: fnm_k
integer(kind=INT_KIND) :: idst(nst),idev(nev),idlo(nlo)
real    :: weig(nlo)
...
integer(kind=INT_KIND)::ndat,nael
real(SP) :: ker0

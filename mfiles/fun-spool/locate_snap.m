...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% LOADING FILES AND READING PARAMETERS
LenFD=3;
str_cm='#';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% CASE FOR THE PARAMETER DOSE NOT EXIST
indx=find(snap_subc==-1);% -1 denotes the parameter dose not exit, need to be calculated
...
for n_k=n_k1:n_k2
    ...
    i=find(ulist>=ni*n_i+1 & ulist<=ni*(n_i+1)); % processing index for each thread 
    ...
    snapinfo(nthd).indxc=snapinfo(nthd).indxe-snapinfo(nthd).indxs+1;% numbers of grids for each thread 

    snapinfo(nthd).wsubs=[ ulist(i(1))-ni*n_i, ...
                           vlist(j(1))-nj*n_j, ...
                           wlist(k(1))-nk*n_k]+LenFD;% numbers of grids for each thread
    snapinfo(nthd).wsubc=snapinfo(nthd).indxc;% numbers of grids for each thread
    snapinfo(nthd).wsubt=snap_subt.*gsubt;% numbers of grids for each thread

    r=find(ilist>=ni*n_i+1 & ilist<=ni*(n_i+1));
    s=find(jlist>=nj*n_j+1 & jlist<=nj*(n_j+1));
    t=find(klist>=nk*n_k+1 & klist<=nk*(n_k+1));% grid index for each thread
